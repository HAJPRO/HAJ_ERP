...
const { is_modal, card_id, item } = storeToRefs(store_spinning);
...
const sendReason = async () => {
  await store_spinning.cancelSendReason({
    id: card_id.value,
...
const isConfirm = () => {
  store_spinning.isConfirmModal({ is_modal: true });
};
</script>
<template>
  <el-dialog
      ...
      <div class="shadow-md rounded min-h-[15px]">
        ...
        >
          ...
          fixed="right"
            ...
            align="center"
            ><template #default="scope">
              {{ String(scope.row.spinning_delivery_time).substring(0, 10) }}
            </template>
          </el-table-column>
        </el-table>
      </div>
