...
const emit = defineEmits(['WeavingLength'])
const items = ref([])
const weaving_length = ref()
const getAllWeaving = async () => {
  const loader = loading.show()
  const data = await SaleLegalService.getAllWeaving();
  items.value = data.data
  weaving_length.value = items.value.length;
  emit("WeavingLength", weaving_length.value);
  loader.hide()
};
